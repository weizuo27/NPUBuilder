...
def constructIPTable(IPs, BRAM_budget, DSP_budget, FF_budget, LUT_budget, BW_budget, layerQueue, explore_IP_types, pipelineLength):
    ...
    for ip in IPs:
        ...
        elif ip.type not in layerQueue:
            IP_num = 0
        else:
            IP_num = min(min(BRAM_budget_local/ip.BRAM, DSP_budget_local/ip.DSP, FF_budget_local/ip.FF, LUT_budget_local/ip.LUT, BW_budget_local/ip.BW), len(layerQueue[ip.type]))
            IP_num = min(IP_num, pipelineLength)
#        print BRAM_budget_local, DSP_budget_local, FF_budget_local, LUT_budget_local, BW_budget_local
#        print ip.BRAM, ip.DSP, ip.FF, ip.BW
...
def resourceConstr(layer, ip):
    ...
    if layer.name == "conv1_1":
        print "in_depth", in_depth, in_height, in_width, kh, S
    ...
    const.append(math.ceil(out_width * math.ceil(float(out_depth)/32) ))
...
def genIPTablePerLayer(IP_table, layerQueue, hw_layers):
    ...
    print "generate IP table per layer"
    ret = dict()
            ...
            for l in layerQueue[layerType]:
                ...
                for ip in IP_queue:
                    if layerType != "Convolution" and layerType != "Convolution_g":
                        ...
                        print const, ip.paramList[2], ip.paramList[3]
                ...
                assert sum(ret[l]) > 0, "For layer " + l.name + ", there is no available IP"
                if l.name == "conv1_1":
                    print "ret[l]", ret[l] 
    return ret
...
#            y = [plt_idx] * len(row_plt)
#            plt_idx += 1
#            ax.scatter(row_plt, y)
#            for i in range(len(row_plt)):
#                ax.annotate(txt[i], (row_plt[i], y[i]))
#    plt.show()
